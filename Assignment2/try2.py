...
ciphertext_binary=hex_to_binary(ciphertext_input)
key_binary=hex_to_binary(key_input)
...
for i in range(64):
    ciphertext_after_ip[i]=int(ciphertext_binary[ip_table[i+1]-1])
...
for round in range(16):
    ...
    round_key= subkeys[15-round] #round_key_compressor(key) #Chnaging round key to array
    rk=""
    for i in round_key:
        rk+=str(i)
